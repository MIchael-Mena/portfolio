...
export class ExperienceService {
  ...
  get experiences(): Observable<WorkData[] | EducationData[]> {
    return this.isWorkingDatabase
      ? this.http.get<WorkData[]>(this.apiUrl)
      : this.http.get<EducationData[]>(this.apiUrl);
  }

  get expOrderByInitialDate(): Observable<WorkData[] | EducationData[]> {
    const url = this.apiUrl + '?_sort=initialDate&_order=desc';
    return this.isWorkingDatabase
      ? this.http.get<WorkData[]>(url)
      : this.http.get<EducationData[]>(url);
  }

  addExperience(
    experience: WorkData | EducationData
  ): Observable<WorkData | EducationData> {
    return this.http.post<WorkData | EducationData>(
      this.apiUrl + '/create',
      experience,
      httpOptions
    );
  ...
  updateExperience(
    ...
    return this.http.put<WorkData | EducationData>(
      url,
      experience,
      httpOptions
    );
