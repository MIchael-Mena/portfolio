...
import { UiEditFormService } from 'src/app/pages/about-me/services/uiEditForm.service';
import { FormExperience } from '../../models/FormExperience';
...
import { ButtonSettings } from '../../../../shared/components/button-confirm/ButtonSettings';
...
export class AddExperienceComponent implements OnChanges, OnInit {
  ...
  @Output() onUpdateExperience: EventEmitter<ExperienceData> =
    new EventEmitter();
  ...
  private onToggleEdit() {
    this.uiEditFormService
      .onToggle()
      .subscribe((experience: ExperienceData) => {
  ...
  private setDate(experience: ExperienceData): void {
    ...
    this.initialDateSettings = {
      ...datePickerClean,
      dateToSet: experience.initialDate,
      maxDate: experience.finalDate,
    };
  }
  ...
  public setInitialControlDate(controlDate: FormControl<any>): void {
    ...
    this.initialDateSettings = {
      ...this.initialDateSettings,
      dateToSet: controlDate.value,
    };
    this.finalDateSettings = {
      ...this.finalDateSettings,
      minDate: controlDate.value,
    };
  }
  ...
  public setPresentDate(checkBox: any): void {
    ...
    this.initialDateSettings = {
      ...this.initialDateSettings,
      maxDate: this.form.controls['finalDate'].value,
    };
  }
  ...
  private updateExperience(experience: ExperienceData): void {
    const anExperience =
      this.formExperience!.reverseParseFromExperienceData(experience);
    ...
    this.experienceService.addExperience(anExperience).subscribe({
      next: (response: WorkData | EducationData) => {
        ...
        this.onAddExperience.emit(
          this.formExperience!.parseToExperienceData(response)
        );
        this.resetForm();
      },
      error: (error: any) => {
  ...
  private formatDate(aDate: any): string {
    return typeof aDate === 'string' || aDate === null
      ? aDate
